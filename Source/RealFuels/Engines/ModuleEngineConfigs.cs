...
namespace ModularFuelSystem
{
    ...
    public class ModuleEngineConfigs : PartModule, IPartCostModifier, IPartMassModifier
    {
        ...
        private IEnumerator<object> HideB9PSInFlightSelector_Coroutine(PartModule module)
        {
            yield return null;
            module.Events["ShowSubtypesWindow"].guiActive = false;
        }
        ...
        public int DoForEachSymmetryCounterpart(Action<ModuleEngineConfigs> action)
        {
            ...
            {
                Part p = part.symmetryCounterparts[j];
                if (p == part)
                    continue;
		...
		private static readonly KSPe.Util.Log.Logger log = KSPe.Util.Log.Logger.CreateForType<ModuleEngineConfigs>(true);
		static ModuleEngineConfigs()
		{
			log.level =
#if DEBUG
